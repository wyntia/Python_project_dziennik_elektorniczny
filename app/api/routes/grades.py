...
async def add_grade(grade: schemas.GradeCreate, db: AsyncSession = Depends(get_db)):
    """Wystawia nową ocenę. Walidacja wartości odbywa się automatycznie w schemacie."""
...
@router.patch("/{grade_id}", response_model=schemas.Grade)
async def edit_grade(grade_id: int, grade_data: schemas.GradeUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_grade(grade_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {"message": f"Ocena o ID {grade_id} została pomyślnie usunięta."}